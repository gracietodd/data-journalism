...
# Load data and store as objects
...
#In buyer_annual_by_year, there are up to six rows for each pharmacy (one per year), while buyer_totals only has one row for each pharmacy. I would guess that there are more records for buyer_addresses than buyer_totals because there are pharmacies included in buyer_addresses that did not purchase any pills, so are not in buyer_totals. 
...
buyer_totals_addresses <- buyer_totals %>%
  left_join(buyer_addresses, by= "buyer_dea_no") %>%
  arrange(desc(total_pills))

#VA Consolidated Mail Outpatient Pharmacy, located in Charleston, SC. This pharmacy is a mail-order prescription filler for veterans, and fills a vast number of prescriptions, which is the likely reason for so many pills. 
...
maryland_buyer_totals <- buyer_annual %>%
  left_join(buyer_addresses, by="buyer_dea_no") %>%
...
# Rakesh K. Mathur, MD with 337,450 total pills, which is over 100,000 more pills than the next practitioner. He was suspended for "prescribing of opioid medications while failing to use reasonable safeguards or to exercise due diligence." He also was permanently banned prescribing medications. He is now on probation. I would make sure it was the same doctor by checking the DEA number from the original data.  
...
buyer_totals %>%
  ...
  summarise(all_pills = sum(total_pills)) %>%
  left_join(state_population, by ="buyer_state") %>%
  mutate(pills_per_person = all_pills / population_average) %>%
  arrange(desc(pills_per_person))

#West Virginia with 464.9 pills.
