...
ggplot(maryland_pills_per_year) +
  geom_bar(stat="identity", aes(year, dosage_unit), fill="2") +
  labs(x="Year", y="Total pills", title= "Opioid shipments to Maryland increase through 2011, steadily decline", caption = "Source: DEA ARCOS database, via Washington Post") +
...
```{r}
# The overall Maryland trend is somewhat different than the Baltimore City data - in Baltimore City the number of pills fell from 2006 then climbed steadily through 2012. In Maryland, the trend was the amount of pills increasing through 2006 to 2011, then decreasing. 
...
```{r}
pills_by_state <-arcos_county_pills_per_year %>%
  clean_names() %>%
  filter(year=="2012") %>%
  group_by(buyer_state) %>%
  summarise(dosage_unit=sum(dosage_unit)) %>%
  select(dosage_unit, buyer_state)
...
pills_by_pop <- state_population_2012 %>%
  inner_join(pills_by_state, by=("buyer_state")) 
...
#ggplot(pills_by_pop) +
 ...
 # labs(x="2012 Population", y="Total pills in 2012", title="tbd", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_state") +
#  scale_y_continuous(labels = comma) +
 ...
 # geom_text_repel(aes(population, dosage_unit, label=buyer_state),
    #subset(pills_by_pop, population > 10,000,000))
```
