...
ggplot(pills_population_income) +
  ...
  scale_x_continuous(labels = comma)  +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x="Average Pills Per Person, Per Year", y="Median Household Income", title="Slight negative correlation between median household income and prescribed opioids", caption = "Source: DEA ARCOS database, via Washington Post")
```
...
Organized into quintiles, the higher the median household income of a group is, the less average annual opiods the group received. A scatterplot shows a negative correlation - it doesn't look strong, but certainly noticebale. In calculating the correlation coefficient, we find it is a weak negative correlation (r= -0.3). The p-value is less than 0.05 so this correlation coefficient is statistically significant. 
