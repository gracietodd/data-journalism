...
```{r}
#The pill table includes data on Puerto Rico, Guam, Virgin Islands, and other targets of American Imperialism. There are also a couple counties in the U.S. - one in Arkansas, and the other in Alaska- included in this table but not the population table. Perhaps these counties were spelled or typed out slightly differently in the two tables. They could cause problems when analyzing state-level data in Arkansas or Alaska, or many other analyses larger than a single county. 
```
...
```{r}
# Define API Key
census_api_key("b663f812c68ea8737376ad2d740a48daa2488aed")
...
geo_pills_population_state_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill= 'Pills per Person, 2009', title="Varying opioid shipments, state to state", subtitle= "Appalachian region hit hard in 2009", caption = "Source: ARCOS database, Washington Post") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Yearly data paints grim picture of an opioid crisis worsening from 2009-2014", subtitle = "Most states received more pills over time", caption = "Source: ARCOS database, Washington Post") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
#pills per county population, 2009
...
#As with the data we read about this week on lead poisoning, the county-level data gives different insight than broader analyses. This shows that certain counties have been hit hard even in states that haven't been (relatively). And it highlights the problem in rural counties out West, though this is partially because the counties are larger and stand out more. 
...
geo_pills_population_county_maryland %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Pills per person in Maryland counties, 2006-2014", subtitle = "Opioid pills shipped to Maryland increase through 2011, then taper off slowly", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)

#More pills per person came to Maryland as years passed, 2006-2014. Generally, counties that received the most pills per person in 2006 compared to other counties, still received a relatively greater amount throughout the years, even as shipments increased in most counties through 2011. There are exceptions to each of these general trends. 
