...
#column for in or out of state distribution 
...
```{r}
#summarize including median pills per shipment 
...
mingo %>%
  ...
  summarise(pipeline = sum(dosage_unit)) %>%
  arrange(desc(pipeline))

#the combination that processed the most pills was Actavis Pharma, Inc. to Miami-Luken to Tug Valley Pharmacy, LLC. 
...
Answer: I would create a new column called cardinal_combined or something of that nature. To do so, I would use a case_when() function to parse out rows where the reporter_name included "CARDINAL HEALTH." Then, I could use these rows (determined by the case when function) to analyze combined Cardinal Health data (from all three locations).
