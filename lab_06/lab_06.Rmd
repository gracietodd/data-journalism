...
```{r}
mingo_county_pills_per_year <- summarized_county_annual(key = key) %>%
  clean_names()
```
...
```{r}
  ...
  geom_bar(stat="identity", aes(year, dosage_unit), fill="royal blue") +
  labs(x="Year", y="Total pills", title="In Mingo County, opioid shipments peaked in 2011", subtitle = "Total pills shipped to Mingo County by year", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014)) +
  scale_y_continuous(labels = comma)
...
```{r}
west_virginia_pills_per_year <- summarized_county_annual(key = key) %>%
  clean_names()
```


```{r}
west_virginia_county_data <- west_virginia_pills_per_year %>%
  filter(buyer_state == "WV", year=="2012") %>%
  select(buyer_county, dosage_unit)
```
...
```{r}
west_virginia_population_2012 <- west_virginia_county_data %>%
...
```{r}
ggplot(west_virginia_population_2012) +
  geom_point(aes(population, dosage_unit)) +
  labs(x="2012 Population", y="Total pills in 2012", title="West Virginia opioid shipments by county 2012", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_county") +
scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  geom_smooth(aes(population, dosage_unit), method = "lm", se = FALSE) +
  geom_text_repel(aes(population, dosage_unit, label=buyer_county),
                  subset(west_virginia_population_2012, population > 50000))
